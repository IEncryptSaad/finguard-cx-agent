...
@router.get('/plugins')
def plugins(user=Depends(require_demo_admin_read('settings:read'))): return plugin_catalog()
@router.get('/workflows', response_model=list[Workflow]|dict)
def workflows(page:int=1,page_size:int=50,paginated:bool=False,user=Depends(require_demo_admin_read('workflow:read'))): return _page(list_workflows(),page,page_size,'updated_at','desc',paginated)
...
def feedback(page:int=1,page_size:int=50,paginated:bool=False,user=Depends(require_demo_admin_read('analytics:read'))): return _page(list_classifications(),page,page_size,'created_at','desc',paginated)
...
def roadmap(type: str|None=None,page:int=1,page_size:int=50,paginated:bool=False,user=Depends(require_demo_admin_read('roadmap:read'))): return _page(list_product_items(type),page,page_size,'updated_at','desc',paginated)
...
def marketplace(user=Depends(require_demo_admin_read('marketplace:read'))): return list_marketplace()
...
def settings_get(user=Depends(require_demo_admin_read('settings:read'))): return get_app_settings()
