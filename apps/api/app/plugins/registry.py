...
from app.plugins.examples import DemoWebhookIntegration
...
registry.register(DemoWebhookIntegration())

def plugin_catalog() -> list[dict]:
    return [{"name": p.metadata.name, "enabled": p.metadata.enabled, "description": p.metadata.description, "type": p.__class__.__name__} for p in registry.all()]
