-- Automation platform schema extension. Safe additive tables for Supabase/Postgres deployments.
create table if not exists workflows (id uuid primary key, name text not null, trigger text not null, conditions jsonb not null default '[]', actions jsonb not null default '[]', retry_policy jsonb not null default '{}', status text not null default 'draft', created_at timestamptz not null default now(), updated_at timestamptz not null default now());
create table if not exists workflow_executions (id uuid primary key, workflow_id uuid references workflows(id), status text not null, attempts integer not null default 1, input jsonb not null default '{}', output jsonb not null default '{}', error text, started_at timestamptz not null default now(), finished_at timestamptz);
